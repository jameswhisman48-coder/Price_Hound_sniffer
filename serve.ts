...
for (let attempt = 1; ; attempt++) {
  ...
  try {
    Bun.serve({
      port: PORT,
      hostname: HOST,
      // Default idle timeout is 10 s; the Kroger price refresh makes ~10
      // sequential API calls with delays and can take 15+ s. Keep the server
      // patient enough for that (and any future crawl endpoints).
      idleTimeout: 120,
      async fetch(req) {
